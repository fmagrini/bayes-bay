...
def add_data_coords(tomo, sources, receivers):
    ...
    for icoord, (isource, ireceiver) in enumerate(
        ...
        data_coords[icoord] = np.concatenate((sources[isource], receivers[ireceiver]))
...
tomo = SeismicTomography(
    cell_size=0.005, lonmin=-1, lonmax=1, latmin=-1, latmax=1, regular_grid=True
)
...
sources, receivers = get_sources_and_receivers(n_sources_per_side=10, n_receivers=50)
...
inversion = bb.BayesianInversion(
    parameterization=parameterization, log_likelihood=log_likelihood, n_chains=1
)
# %%
inversion.run(
    sampler=None,
